...
train_data = load_data("dataset/train1.source", "dataset/train1.target")
valid_data = load_data("dataset/val1.source", "dataset/val1.target")
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if metrics['bleu'] > self.best_bleu:
            ...
            model.save_weights('./save/best_model_%d.weights' % (int(epoch)))  # 保存模型
    ...
    def evaluate(self, data, topk=1):
        ...
        ans_title, ans_pred_title = "", ""
        for title, content in tqdm(data):
            total += 1
            title = ' '.join(title).lower()
            ans_title = ''.join(title).lower()
            pred_title = ' '.join(autotitle.generate(content, topk)).lower()
            ans_pred_title = ''.join(autotitle.generate(content, topk)).lower()
            if pred_title.strip():
